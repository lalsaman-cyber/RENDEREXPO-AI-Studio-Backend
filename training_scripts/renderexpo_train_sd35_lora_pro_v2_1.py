#!/usr/bin/env python
# RENDEREXPO SD3.5-Large LyCORIS trainer (PRO v2.1, caption-aware, 1024px, rank 16)
#
...
def parse_args():
    p = argparse.ArgumentParser(
        "RENDEREXPO SD3.5 LyCORIS trainer PRO v2.1 (caption-aware, fp16, 1024px, rank 16)"
    )
    ...
    p.add_argument(
        "--output_dir",
        type=str,
        default="outputs/lora/sd35_renderexpo_pro_v2_1",
    ...
    p.add_argument(
        ...
        type=int,
        default=1024,
        help="Training resolution (PRO v2.1 default = 1024)",
    )
    p.add_argument(
        "--train_batch_size",
        type=int,
    ...
    p.add_argument(
        ...
        type=int,
        default=15000,
        help="Total training steps (PRO v2.1 default = 15000)",
    )
    p.add_argument(
        "--learning_rate",
        type=float,
        default=1e-4,
        help="Base learning rate (start LR, PRO v2.1 = 1e-4)",
    )
    p.add_argument(
        "--lr_mid",
        type=float,
        default=4e-5,
        help="Mid-phase learning rate target (PRO v2.1 = 4e-5).",
    )
    p.add_argument(
        "--lr_final",
        type=float,
        default=1.5e-5,
        help="Final-phase learning rate target (PRO v2.1 = 1.5e-5).",
    )
    p.add_argument(
        "--rank",
        type=int,
        default=16,
        help="LyCORIS rank (PRO v2.1 default = 16).",
    )
    p.add_argument(
        "--alpha",
        type=float,
        default=16.0,
        help="LyCORIS alpha (PRO v2.1 default = 16).",
    )
    p.add_argument(
        "--lycoris_algo",
        type=str,
...
def main():
    ...
    print("==========================================")
    print("RENDEREXPO PRO v2.1 – SD3.5 LyCORIS Trainer")
    print("==========================================")
    ...
    print(
        f"Starting PRO v2.1 training for {total_steps} steps (LyCORIS {args.lycoris_algo}, {args.resolution}px, fp16 on GPU)..."
    )
    progress_bar = tqdm(range(total_steps), desc="Training steps")

    transformer.train()

    # sharper noise scale (PRO v2.1)
    noise_scale = 0.25
