...
async def sd35_render(request: SD35Text2ImgRequest):
    """
    Skeleton Text2Img endpoint for SD3.5.

    On LOCAL (CPU) app:
    - Does NOT run SD3.5
    - Does NOT load weights
    - Only:
        * validates LoRA + refiner profile names
        * creates job folder under outputs/{date}/{job_id}/
        * writes meta.json with the planned settings
        * returns job information

    On GPU (RunPod) later:
    - The GPU app will:
        * load SD3.5 into GPU
        * apply LoRAs + refiners
        * actually render the image
        * write real output.png
    """
    ...
    meta: Dict[str, Any] = {
        ...
        # New: store profiles + resolved config
