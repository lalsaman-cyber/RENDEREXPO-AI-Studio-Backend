...
def _dispatch(job_folder: str, meta: Dict[str, Any]) -> Tuple[bool, Dict[str, Any]]:
    ...
    Contract:
      payload = { "job_folder": <ABS_PATH>, "meta": <meta.json dict> }
    """
...
# ---- SD3.5 Job Dispatchers (all share the same dispatch endpoint) ----
...
# ---- Moodboard / Space / Render helpers (router imports expect these names) ----
# They still call the same GPU dispatch endpoint; the "mode" is decided by meta contents.

def dispatch_sd35_moodboard_to_space(job_folder: str, meta: Dict[str, Any]) -> Tuple[bool, Dict[str, Any]]:
    """Dispatch a moodboard -> space generation job to the GPU worker."""
    return _dispatch(job_folder, meta)


def dispatch_space_to_moodboard(job_folder: str, meta: Dict[str, Any]) -> Tuple[bool, Dict[str, Any]]:
    """Dispatch a space -> moodboard extraction job to the GPU worker."""
    return _dispatch(job_folder, meta)


def dispatch_sd35_apply_moodboard_to_render(job_folder: str, meta: Dict[str, Any]) -> Tuple[bool, Dict[str, Any]]:
    """Dispatch applying a moodboard to an existing render job to the GPU worker."""
    return _dispatch(job_folder, meta)


def dispatch_sd35_apply_moodboard_to_space(job_folder: str, meta: Dict[str, Any]) -> Tuple[bool, Dict[str, Any]]:
    """Dispatch applying a moodboard to a space job to the GPU worker."""
    return _dispatch(job_folder, meta)


def dispatch_sd35_render_to_moodboard(job_folder: str, meta: Dict[str, Any]) -> Tuple[bool, Dict[str, Any]]:
    """Dispatch a render -> moodboard extraction job to the GPU worker."""
    return _dispatch(job_folder, meta)
